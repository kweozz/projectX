...
uniform float uStreak;     // vertical smear, CSS px
...
void main(){
  ...
  float off = refract * uAmp;

  // Vertical smear (a touch) + chromatic dispersion, averaged.
  vec3 col = vec3(0.0);
  const int N = 5;
  for (int i = 0; i < N; i++){
...
export default function FlutedImage({
  ...
  const p = useRef({ fluteWidth, amp, edge, shine, streak, chroma, sweep, animate, objectPositionX, objectPositionY })
  p.current = { fluteWidth, amp, edge, shine, streak, chroma, sweep, animate, objectPositionX, objectPositionY }
  ...
  useEffect(() => {
    ...
    const u = {
      ...
      streak: U('uStreak'), chroma: U('uChroma'), sweep: U('uSweep'), time: U('uTime'), focus: U('uFocus'),
    }
